...
class OrderByItemRating(BaseModel):
    """
    Параметры сортировки
    """ # noqa: E501
    var_field: StrictStr = Field(description="Поле для сортировки:   - `feedbackRating` — Рейтинг товара по отзывам   - `feedbackCount` — Все отзывы за период   - `fiveStar` — Отзывы 5 звёзд   - `fourStar` — Отзывы 4 звезды   - `threeStar` — Отзывы 3 звезды   - `twoStar` — Отзывы 2 звезды   - `oneStar` — Отзывы 1 звезда   - `disqualified` — Отзывы, исключённые из рейтинга ", alias="field", json_schema_extra={"examples": ["feedbackCount"]})
    mode: StrictStr = Field(description="Порядок сортировки:   - `asc` — по возрастанию   - `desc` — по убыванию ", json_schema_extra={"examples": ["desc"]})
    __properties: ClassVar[List[str]] = ["field", "mode"]

    @field_validator('var_field')
    def var_field_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['feedbackRating', 'feedbackCount', 'fiveStar', 'fourStar', 'threeStar', 'twoStar', 'oneStar', 'disqualified']):
            raise ValueError("must be one of enum values ('feedbackRating', 'feedbackCount', 'fiveStar', 'fourStar', 'threeStar', 'twoStar', 'oneStar', 'disqualified')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OrderByItemRating from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OrderByItemRating from a dict"""
        if obj is None:
            return None
