...
class ApiV3WarehousesWarehouseIdPutRequest(BaseModel):
    ...
    office_id: Annotated[int, Field(strict=True, ge=1)] = Field(description="ID [склада WB](./work-with-products#tag/Sklady-prodavca/paths/~1api~1v3~1offices/get).<br>Нельзя привязывать склад WB, который уже используется.<br>Можно менять не чаще одного раза в сутки", alias="officeId", json_schema_extra={"examples": [15]})
