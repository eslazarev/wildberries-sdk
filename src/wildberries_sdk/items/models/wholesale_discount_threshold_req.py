...
class WholesaleDiscountThresholdReq(BaseModel):
    """
    WholesaleDiscountThresholdReq
    """ # noqa: E501
    min_quantity: StrictInt = Field(description="Минимальное количество единиц товара для скидки", alias="minQuantity", json_schema_extra={"examples": [10]})
    wholesale_discount: StrictInt = Field(description="Скидка, %.<br>Чтобы удалить скидку, укажите `0`. Такой запрос одновременно удалит скидки на этом уровне и на всех более высоких уровнях `level` — вне зависимости от указания их в запросе", alias="wholesaleDiscount", json_schema_extra={"examples": [10]})
    level: StrictInt = Field(description="Уровень скидки", json_schema_extra={"examples": [1]})
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of WholesaleDiscountThresholdReq from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of WholesaleDiscountThresholdReq from a dict"""
        if obj is None:
            return None
