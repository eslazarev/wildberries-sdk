...
class ApiV3OrdersStatusPost200ResponseOrdersInner(BaseModel):
    ...
    is_cancellable: Optional[StrictBool] = Field(default=None, description="Доступна ли [отмена](./orders-fbs#tag/Sborochnye-zadaniya-FBS/paths/~1api~1v3~1orders~1%7BorderId%7D~1cancel/patch) сборочного задания: - `false` — недоступна - `true` — доступна ", alias="isCancellable", json_schema_extra={"examples": [False]})
