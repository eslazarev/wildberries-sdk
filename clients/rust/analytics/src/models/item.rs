/*
 * Аналитика и данные
 *
 * <div class=\"description_important\">   Узнать больше об аналитике и данных можно в <a href=\"https://seller.wildberries.ru/instructions/ru/ru/subcategory/seller-analytics\">справочном центре</a> </div>  <div class=\"api-block\">  В разделе описаны методы получения:   1. [Воронки продаж](/openapi/analytics#tag/salesFunnel)   2. [Ленты заказов](/openapi/analytics#tag/orderFeed)   3. [Поисковых запросов по вашим товарам](/openapi/analytics#tag/searchQueriesForYourItems)   4. [Истории остатков](/openapi/analytics#tag/stocksReport)   5. [Оценки товара](/openapi/analytics#tag/itemRating)   6. [Аналитики продавца в формате CSV](/openapi/analytics#tag/sellerAnalyticsCsv)  </div> 
 *
 * The version of the OpenAPI document: analytics
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct Item {
    /// Артикул WB
    #[serde(rename = "nmId")]
    pub nm_id: i64,
    /// Название карточки товара
    #[serde(rename = "title")]
    pub title: String,
    /// Артикул продавца
    #[serde(rename = "vendorCode")]
    pub vendor_code: String,
    /// Бренд
    #[serde(rename = "brandName")]
    pub brand_name: String,
    /// ID предмета
    #[serde(rename = "subjectId")]
    pub subject_id: u64,
    /// Название предмета
    #[serde(rename = "subjectName")]
    pub subject_name: String,
    /// Ярлыки
    #[serde(rename = "tags")]
    pub tags: Vec<models::Tag>,
    /// Оценка карточки
    #[serde(rename = "productRating")]
    pub product_rating: f64,
    /// Оценка пользователей
    #[serde(rename = "feedbackRating")]
    pub feedback_rating: f64,
    #[serde(rename = "stocks")]
    pub stocks: Box<models::ItemStocks>,
}

impl Item {
    pub fn new(nm_id: i64, title: String, vendor_code: String, brand_name: String, subject_id: u64, subject_name: String, tags: Vec<models::Tag>, product_rating: f64, feedback_rating: f64, stocks: models::ItemStocks) -> Item {
        Item {
            nm_id,
            title,
            vendor_code,
            brand_name,
            subject_id,
            subject_name,
            tags,
            product_rating,
            feedback_rating,
            stocks: Box::new(stocks),
        }
    }
}

