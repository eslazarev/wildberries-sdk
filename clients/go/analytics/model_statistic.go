/*
Аналитика и данные

<div class=\"description_important\">   Узнать больше об аналитике и данных можно в <a href=\"https://seller.wildberries.ru/instructions/ru/ru/subcategory/seller-analytics\">справочном центре</a> </div>  <div class=\"api-block\">  В разделе описаны методы получения:   1. [Воронки продаж](/openapi/analytics#tag/salesFunnel)   2. [Ленты заказов](/openapi/analytics#tag/orderFeed)   3. [Поисковых запросов по вашим товарам](/openapi/analytics#tag/searchQueriesForYourItems)   4. [Истории остатков](/openapi/analytics#tag/stocksReport)   5. [Оценки товара](/openapi/analytics#tag/itemRating)   6. [Аналитики продавца в формате CSV](/openapi/analytics#tag/sellerAnalyticsCsv)  </div> 

API version: analytics
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package analytics

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the Statistic type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Statistic{}

// Statistic struct for Statistic
type Statistic struct {
	// Даты периода
	Period DatePeriod `json:"period"`
	// Количество переходов в карточку товара
	OpenCount int32 `json:"openCount"`
	// Положили в корзину, шт.
	CartCount int32 `json:"cartCount"`
	// Заказали товаров, шт.
	OrderCount int32 `json:"orderCount"`
	// Заказали на сумму
	OrderSum int32 `json:"orderSum"`
	// Выкупили товаров, шт.
	BuyoutCount int32 `json:"buyoutCount"`
	// Выкупили на сумму
	BuyoutSum int32 `json:"buyoutSum"`
	// Отменили и вернули товаров, шт.
	CancelCount int32 `json:"cancelCount"`
	// Отменили и вернули на сумму
	CancelSum int32 `json:"cancelSum"`
	// Средняя цена
	AvgPrice int32 `json:"avgPrice"`
	// Среднее количество заказов в день, шт.
	AvgOrdersCountPerDay float32 `json:"avgOrdersCountPerDay"`
	// Доля в выручке
	ShareOrderPercent float32 `json:"shareOrderPercent"`
	// Добавили в **Отложенные**
	AddToWishlist int32 `json:"addToWishlist"`
	TimeToReady StatisticTimeToReady `json:"timeToReady"`
	// Локальные заказы в рамках одного региона
	LocalizationPercent int32 `json:"localizationPercent"`
	WbClub StatisticWbClub `json:"wbClub"`
	Conversions StatisticConversions `json:"conversions"`
}

type _Statistic Statistic

// NewStatistic instantiates a new Statistic object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewStatistic(period DatePeriod, openCount int32, cartCount int32, orderCount int32, orderSum int32, buyoutCount int32, buyoutSum int32, cancelCount int32, cancelSum int32, avgPrice int32, avgOrdersCountPerDay float32, shareOrderPercent float32, addToWishlist int32, timeToReady StatisticTimeToReady, localizationPercent int32, wbClub StatisticWbClub, conversions StatisticConversions) *Statistic {
	this := Statistic{}
	this.Period = period
	this.OpenCount = openCount
	this.CartCount = cartCount
	this.OrderCount = orderCount
	this.OrderSum = orderSum
	this.BuyoutCount = buyoutCount
	this.BuyoutSum = buyoutSum
	this.CancelCount = cancelCount
	this.CancelSum = cancelSum
	this.AvgPrice = avgPrice
	this.AvgOrdersCountPerDay = avgOrdersCountPerDay
	this.ShareOrderPercent = shareOrderPercent
	this.AddToWishlist = addToWishlist
	this.TimeToReady = timeToReady
	this.LocalizationPercent = localizationPercent
	this.WbClub = wbClub
	this.Conversions = conversions
	return &this
}

// NewStatisticWithDefaults instantiates a new Statistic object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewStatisticWithDefaults() *Statistic {
	this := Statistic{}
	return &this
}

// GetPeriod returns the Period field value
func (o *Statistic) GetPeriod() DatePeriod {
	if o == nil {
		var ret DatePeriod
		return ret
	}

	return o.Period
}

// GetPeriodOk returns a tuple with the Period field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetPeriodOk() (*DatePeriod, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Period, true
}

// SetPeriod sets field value
func (o *Statistic) SetPeriod(v DatePeriod) {
	o.Period = v
}

// GetOpenCount returns the OpenCount field value
func (o *Statistic) GetOpenCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.OpenCount
}

// GetOpenCountOk returns a tuple with the OpenCount field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetOpenCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OpenCount, true
}

// SetOpenCount sets field value
func (o *Statistic) SetOpenCount(v int32) {
	o.OpenCount = v
}

// GetCartCount returns the CartCount field value
func (o *Statistic) GetCartCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.CartCount
}

// GetCartCountOk returns a tuple with the CartCount field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetCartCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CartCount, true
}

// SetCartCount sets field value
func (o *Statistic) SetCartCount(v int32) {
	o.CartCount = v
}

// GetOrderCount returns the OrderCount field value
func (o *Statistic) GetOrderCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.OrderCount
}

// GetOrderCountOk returns a tuple with the OrderCount field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetOrderCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrderCount, true
}

// SetOrderCount sets field value
func (o *Statistic) SetOrderCount(v int32) {
	o.OrderCount = v
}

// GetOrderSum returns the OrderSum field value
func (o *Statistic) GetOrderSum() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.OrderSum
}

// GetOrderSumOk returns a tuple with the OrderSum field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetOrderSumOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrderSum, true
}

// SetOrderSum sets field value
func (o *Statistic) SetOrderSum(v int32) {
	o.OrderSum = v
}

// GetBuyoutCount returns the BuyoutCount field value
func (o *Statistic) GetBuyoutCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.BuyoutCount
}

// GetBuyoutCountOk returns a tuple with the BuyoutCount field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetBuyoutCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.BuyoutCount, true
}

// SetBuyoutCount sets field value
func (o *Statistic) SetBuyoutCount(v int32) {
	o.BuyoutCount = v
}

// GetBuyoutSum returns the BuyoutSum field value
func (o *Statistic) GetBuyoutSum() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.BuyoutSum
}

// GetBuyoutSumOk returns a tuple with the BuyoutSum field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetBuyoutSumOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.BuyoutSum, true
}

// SetBuyoutSum sets field value
func (o *Statistic) SetBuyoutSum(v int32) {
	o.BuyoutSum = v
}

// GetCancelCount returns the CancelCount field value
func (o *Statistic) GetCancelCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.CancelCount
}

// GetCancelCountOk returns a tuple with the CancelCount field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetCancelCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CancelCount, true
}

// SetCancelCount sets field value
func (o *Statistic) SetCancelCount(v int32) {
	o.CancelCount = v
}

// GetCancelSum returns the CancelSum field value
func (o *Statistic) GetCancelSum() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.CancelSum
}

// GetCancelSumOk returns a tuple with the CancelSum field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetCancelSumOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CancelSum, true
}

// SetCancelSum sets field value
func (o *Statistic) SetCancelSum(v int32) {
	o.CancelSum = v
}

// GetAvgPrice returns the AvgPrice field value
func (o *Statistic) GetAvgPrice() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.AvgPrice
}

// GetAvgPriceOk returns a tuple with the AvgPrice field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetAvgPriceOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AvgPrice, true
}

// SetAvgPrice sets field value
func (o *Statistic) SetAvgPrice(v int32) {
	o.AvgPrice = v
}

// GetAvgOrdersCountPerDay returns the AvgOrdersCountPerDay field value
func (o *Statistic) GetAvgOrdersCountPerDay() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.AvgOrdersCountPerDay
}

// GetAvgOrdersCountPerDayOk returns a tuple with the AvgOrdersCountPerDay field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetAvgOrdersCountPerDayOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AvgOrdersCountPerDay, true
}

// SetAvgOrdersCountPerDay sets field value
func (o *Statistic) SetAvgOrdersCountPerDay(v float32) {
	o.AvgOrdersCountPerDay = v
}

// GetShareOrderPercent returns the ShareOrderPercent field value
func (o *Statistic) GetShareOrderPercent() float32 {
	if o == nil {
		var ret float32
		return ret
	}

	return o.ShareOrderPercent
}

// GetShareOrderPercentOk returns a tuple with the ShareOrderPercent field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetShareOrderPercentOk() (*float32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ShareOrderPercent, true
}

// SetShareOrderPercent sets field value
func (o *Statistic) SetShareOrderPercent(v float32) {
	o.ShareOrderPercent = v
}

// GetAddToWishlist returns the AddToWishlist field value
func (o *Statistic) GetAddToWishlist() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.AddToWishlist
}

// GetAddToWishlistOk returns a tuple with the AddToWishlist field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetAddToWishlistOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AddToWishlist, true
}

// SetAddToWishlist sets field value
func (o *Statistic) SetAddToWishlist(v int32) {
	o.AddToWishlist = v
}

// GetTimeToReady returns the TimeToReady field value
func (o *Statistic) GetTimeToReady() StatisticTimeToReady {
	if o == nil {
		var ret StatisticTimeToReady
		return ret
	}

	return o.TimeToReady
}

// GetTimeToReadyOk returns a tuple with the TimeToReady field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetTimeToReadyOk() (*StatisticTimeToReady, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TimeToReady, true
}

// SetTimeToReady sets field value
func (o *Statistic) SetTimeToReady(v StatisticTimeToReady) {
	o.TimeToReady = v
}

// GetLocalizationPercent returns the LocalizationPercent field value
func (o *Statistic) GetLocalizationPercent() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.LocalizationPercent
}

// GetLocalizationPercentOk returns a tuple with the LocalizationPercent field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetLocalizationPercentOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LocalizationPercent, true
}

// SetLocalizationPercent sets field value
func (o *Statistic) SetLocalizationPercent(v int32) {
	o.LocalizationPercent = v
}

// GetWbClub returns the WbClub field value
func (o *Statistic) GetWbClub() StatisticWbClub {
	if o == nil {
		var ret StatisticWbClub
		return ret
	}

	return o.WbClub
}

// GetWbClubOk returns a tuple with the WbClub field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetWbClubOk() (*StatisticWbClub, bool) {
	if o == nil {
		return nil, false
	}
	return &o.WbClub, true
}

// SetWbClub sets field value
func (o *Statistic) SetWbClub(v StatisticWbClub) {
	o.WbClub = v
}

// GetConversions returns the Conversions field value
func (o *Statistic) GetConversions() StatisticConversions {
	if o == nil {
		var ret StatisticConversions
		return ret
	}

	return o.Conversions
}

// GetConversionsOk returns a tuple with the Conversions field value
// and a boolean to check if the value has been set.
func (o *Statistic) GetConversionsOk() (*StatisticConversions, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Conversions, true
}

// SetConversions sets field value
func (o *Statistic) SetConversions(v StatisticConversions) {
	o.Conversions = v
}

func (o Statistic) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Statistic) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["period"] = o.Period
	toSerialize["openCount"] = o.OpenCount
	toSerialize["cartCount"] = o.CartCount
	toSerialize["orderCount"] = o.OrderCount
	toSerialize["orderSum"] = o.OrderSum
	toSerialize["buyoutCount"] = o.BuyoutCount
	toSerialize["buyoutSum"] = o.BuyoutSum
	toSerialize["cancelCount"] = o.CancelCount
	toSerialize["cancelSum"] = o.CancelSum
	toSerialize["avgPrice"] = o.AvgPrice
	toSerialize["avgOrdersCountPerDay"] = o.AvgOrdersCountPerDay
	toSerialize["shareOrderPercent"] = o.ShareOrderPercent
	toSerialize["addToWishlist"] = o.AddToWishlist
	toSerialize["timeToReady"] = o.TimeToReady
	toSerialize["localizationPercent"] = o.LocalizationPercent
	toSerialize["wbClub"] = o.WbClub
	toSerialize["conversions"] = o.Conversions
	return toSerialize, nil
}

func (o *Statistic) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"period",
		"openCount",
		"cartCount",
		"orderCount",
		"orderSum",
		"buyoutCount",
		"buyoutSum",
		"cancelCount",
		"cancelSum",
		"avgPrice",
		"avgOrdersCountPerDay",
		"shareOrderPercent",
		"addToWishlist",
		"timeToReady",
		"localizationPercent",
		"wbClub",
		"conversions",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varStatistic := _Statistic{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varStatistic)

	if err != nil {
		return err
	}

	*o = Statistic(varStatistic)

	return err
}

type NullableStatistic struct {
	value *Statistic
	isSet bool
}

func (v NullableStatistic) Get() *Statistic {
	return v.value
}

func (v *NullableStatistic) Set(val *Statistic) {
	v.value = val
	v.isSet = true
}

func (v NullableStatistic) IsSet() bool {
	return v.isSet
}

func (v *NullableStatistic) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStatistic(val *Statistic) *NullableStatistic {
	return &NullableStatistic{value: val, isSet: true}
}

func (v NullableStatistic) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStatistic) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


