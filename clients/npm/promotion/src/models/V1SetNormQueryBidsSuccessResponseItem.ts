/* tslint:disable */
/* eslint-disable */
/**
 * Маркетинг и продвижение
 * <div class=\"description_important\">   Узнать больше о маркетинге и продвижении можно в <a href=\"https://seller.wildberries.ru/instructions/category/59d92bd3-6ea0-40f2-b762-ca8835d7d42e?goBackOption=prevRoute&categoryId=479385c6-de01-4b4d-ad4e-ed941e65582e\">справочном центре</a> </div>  <div class=\"api-block\">  Методы маркетинга и продвижения позволяют:   1. Получать информацию о кампаниях [продвижения](/openapi/promotion#tag/campaigns) и [медиакампаниях](/openapi/promotion#tag/media)   2. [Создавать](/openapi/promotion#tag/creatingCampaigns) и [управлять](/openapi/promotion#tag/campaignManagement) кампаниями   3. Управлять [финансами](/openapi/promotion#tag/finances) кампаний   4. Выгружать [статистику](/openapi/promotion#tag/statistics) кампаний продвижения и медиакампаний   5. Работать с [календарём акций](/openapi/promotion#tag/promoCalendar)  Данные синхронизируются с базой раз в 3 минуты. Статусы кампаний меняются раз в минуту. Ставки кампаний меняются раз в 30 секунд.<br><br>  Вы можете протестировать методы продвижения в [песочнице](/sandbox). Также в песочнице доступны [специальные методы](/docs/openapi-other/sandbox-environment#tag/promotion) для управления тестовым балансом  </div> 
 *
 * The version of the OpenAPI document: promotion
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface V1SetNormQueryBidsSuccessResponseItem
 */
export interface V1SetNormQueryBidsSuccessResponseItem {
    /**
     * ID кампании
     */
    advertId: number;
    /**
     * Артикул WB
     */
    nmId: number;
    /**
     * Поисковый кластер — это группа похожих поисковых запросов, по которым покупатели находят товары
     */
    normQuery: string;
    /**
     * Валюта [аккаунта продавца](https://cmp.wildberries.ru/campaigns/finances)
     */
    currency: string;
}

/**
 * Check if a given object implements the V1SetNormQueryBidsSuccessResponseItem interface.
 */
export function instanceOfV1SetNormQueryBidsSuccessResponseItem(value: object): value is V1SetNormQueryBidsSuccessResponseItem {
    if (!('advertId' in value) || value['advertId'] === undefined) return false;
    if (!('nmId' in value) || value['nmId'] === undefined) return false;
    if (!('normQuery' in value) || value['normQuery'] === undefined) return false;
    if (!('currency' in value) || value['currency'] === undefined) return false;
    return true;
}

export function V1SetNormQueryBidsSuccessResponseItemFromJSON(json: any): V1SetNormQueryBidsSuccessResponseItem {
    return V1SetNormQueryBidsSuccessResponseItemFromJSONTyped(json, false);
}

export function V1SetNormQueryBidsSuccessResponseItemFromJSONTyped(json: any, ignoreDiscriminator: boolean): V1SetNormQueryBidsSuccessResponseItem {
    if (json == null) {
        return json;
    }
    return {
        
        'advertId': json['advertId'],
        'nmId': json['nmId'],
        'normQuery': json['normQuery'],
        'currency': json['currency'],
    };
}

export function V1SetNormQueryBidsSuccessResponseItemToJSON(json: any): V1SetNormQueryBidsSuccessResponseItem {
    return V1SetNormQueryBidsSuccessResponseItemToJSONTyped(json, false);
}

export function V1SetNormQueryBidsSuccessResponseItemToJSONTyped(value?: V1SetNormQueryBidsSuccessResponseItem | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'advertId': value['advertId'],
        'nmId': value['nmId'],
        'normQuery': value['normQuery'],
        'currency': value['currency'],
    };
}

