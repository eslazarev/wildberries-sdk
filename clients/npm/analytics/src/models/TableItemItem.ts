/* tslint:disable */
/* eslint-disable */
/**
 * Аналитика и данные
 * <div class=\"description_important\">   Узнать больше об аналитике и данных можно в <a href=\"https://seller.wildberries.ru/instructions/ru/ru/subcategory/seller-analytics\">справочном центре</a> </div>  <div class=\"api-block\">  В разделе описаны методы получения:   1. [Воронки продаж](/openapi/analytics#tag/Voronka-prodazh)   2. [Поисковых запросов по вашим товарам](/openapi/analytics#tag/Poiskovye-zaprosy-po-vashim-tovaram)   3. [Истории остатков](/openapi/analytics#tag/Istoriya-ostatkov)   4. [Оценки товара](/openapi/analytics#tag/Ocenka-tovara)   5. [Аналитики продавца в формате CSV](/openapi/analytics#tag/Analitika-prodavca-CSV)  </div> 
 *
 * The version of the OpenAPI document: analytics
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { TableItemItemAllOfAvgPosition } from './TableItemItemAllOfAvgPosition';
import {
    TableItemItemAllOfAvgPositionFromJSON,
    TableItemItemAllOfAvgPositionFromJSONTyped,
    TableItemItemAllOfAvgPositionToJSON,
    TableItemItemAllOfAvgPositionToJSONTyped,
} from './TableItemItemAllOfAvgPosition';
import type { TableItemItemAllOfPrice } from './TableItemItemAllOfPrice';
import {
    TableItemItemAllOfPriceFromJSON,
    TableItemItemAllOfPriceFromJSONTyped,
    TableItemItemAllOfPriceToJSON,
    TableItemItemAllOfPriceToJSONTyped,
} from './TableItemItemAllOfPrice';

/**
 * 
 * @export
 * @interface TableItemItem
 */
export interface TableItemItem {
    /**
     * Артикул WB
     * @type {number}
     * @memberof TableItemItem
     */
    nmId: number;
    /**
     * Название товара
     * @type {string}
     * @memberof TableItemItem
     */
    name?: string;
    /**
     * Артикул продавца
     * @type {string}
     * @memberof TableItemItem
     */
    vendorCode: string;
    /**
     * Название предмета
     * @type {string}
     * @memberof TableItemItem
     */
    subjectName?: string;
    /**
     * Бренд
     * @type {string}
     * @memberof TableItemItem
     */
    brandName?: string;
    /**
     * URL главного фото карточки товара
     * @type {string}
     * @memberof TableItemItem
     */
    mainPhoto?: string;
    /**
     * Находится ли товар в продвижении в Поисковой выдаче
     * @type {boolean}
     * @memberof TableItemItem
     */
    isAdvertised: boolean;
    /**
     * Искали ли товар по подменному артикулу.<br>
     * Поле будет в ответе при наличии в запросе `includeSubstitutedSKUs` и/или `includeSearchTexts`
     * 
     * @type {boolean}
     * @memberof TableItemItem
     */
    isSubstitutedSKU?: boolean;
    /**
     * Есть ли рейтинг у карточки товара
     * @type {boolean}
     * @memberof TableItemItem
     */
    isCardRated: boolean;
    /**
     * Рейтинг карточки товара
     * @type {number}
     * @memberof TableItemItem
     */
    rating: number;
    /**
     * Рейтинг по отзывам
     * @type {number}
     * @memberof TableItemItem
     */
    feedbackRating: number;
    /**
     * 
     * @type {TableItemItemAllOfPrice}
     * @memberof TableItemItem
     */
    price: TableItemItemAllOfPrice;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    avgPosition: TableItemItemAllOfAvgPosition;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    openCard: TableItemItemAllOfAvgPosition;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    addToCart: TableItemItemAllOfAvgPosition;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    openToCart: TableItemItemAllOfAvgPosition;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    orders: TableItemItemAllOfAvgPosition;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    cartToOrder: TableItemItemAllOfAvgPosition;
    /**
     * 
     * @type {TableItemItemAllOfAvgPosition}
     * @memberof TableItemItem
     */
    visibility: TableItemItemAllOfAvgPosition;
}

/**
 * Check if a given object implements the TableItemItem interface.
 */
export function instanceOfTableItemItem(value: object): value is TableItemItem {
    if (!('nmId' in value) || value['nmId'] === undefined) return false;
    if (!('vendorCode' in value) || value['vendorCode'] === undefined) return false;
    if (!('isAdvertised' in value) || value['isAdvertised'] === undefined) return false;
    if (!('isCardRated' in value) || value['isCardRated'] === undefined) return false;
    if (!('rating' in value) || value['rating'] === undefined) return false;
    if (!('feedbackRating' in value) || value['feedbackRating'] === undefined) return false;
    if (!('price' in value) || value['price'] === undefined) return false;
    if (!('avgPosition' in value) || value['avgPosition'] === undefined) return false;
    if (!('openCard' in value) || value['openCard'] === undefined) return false;
    if (!('addToCart' in value) || value['addToCart'] === undefined) return false;
    if (!('openToCart' in value) || value['openToCart'] === undefined) return false;
    if (!('orders' in value) || value['orders'] === undefined) return false;
    if (!('cartToOrder' in value) || value['cartToOrder'] === undefined) return false;
    if (!('visibility' in value) || value['visibility'] === undefined) return false;
    return true;
}

export function TableItemItemFromJSON(json: any): TableItemItem {
    return TableItemItemFromJSONTyped(json, false);
}

export function TableItemItemFromJSONTyped(json: any, ignoreDiscriminator: boolean): TableItemItem {
    if (json == null) {
        return json;
    }
    return {
        
        'nmId': json['nmId'],
        'name': json['name'] == null ? undefined : json['name'],
        'vendorCode': json['vendorCode'],
        'subjectName': json['subjectName'] == null ? undefined : json['subjectName'],
        'brandName': json['brandName'] == null ? undefined : json['brandName'],
        'mainPhoto': json['mainPhoto'] == null ? undefined : json['mainPhoto'],
        'isAdvertised': json['isAdvertised'],
        'isSubstitutedSKU': json['isSubstitutedSKU'] == null ? undefined : json['isSubstitutedSKU'],
        'isCardRated': json['isCardRated'],
        'rating': json['rating'],
        'feedbackRating': json['feedbackRating'],
        'price': TableItemItemAllOfPriceFromJSON(json['price']),
        'avgPosition': TableItemItemAllOfAvgPositionFromJSON(json['avgPosition']),
        'openCard': TableItemItemAllOfAvgPositionFromJSON(json['openCard']),
        'addToCart': TableItemItemAllOfAvgPositionFromJSON(json['addToCart']),
        'openToCart': TableItemItemAllOfAvgPositionFromJSON(json['openToCart']),
        'orders': TableItemItemAllOfAvgPositionFromJSON(json['orders']),
        'cartToOrder': TableItemItemAllOfAvgPositionFromJSON(json['cartToOrder']),
        'visibility': TableItemItemAllOfAvgPositionFromJSON(json['visibility']),
    };
}

export function TableItemItemToJSON(json: any): TableItemItem {
    return TableItemItemToJSONTyped(json, false);
}

export function TableItemItemToJSONTyped(value?: TableItemItem | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'nmId': value['nmId'],
        'name': value['name'],
        'vendorCode': value['vendorCode'],
        'subjectName': value['subjectName'],
        'brandName': value['brandName'],
        'mainPhoto': value['mainPhoto'],
        'isAdvertised': value['isAdvertised'],
        'isSubstitutedSKU': value['isSubstitutedSKU'],
        'isCardRated': value['isCardRated'],
        'rating': value['rating'],
        'feedbackRating': value['feedbackRating'],
        'price': TableItemItemAllOfPriceToJSON(value['price']),
        'avgPosition': TableItemItemAllOfAvgPositionToJSON(value['avgPosition']),
        'openCard': TableItemItemAllOfAvgPositionToJSON(value['openCard']),
        'addToCart': TableItemItemAllOfAvgPositionToJSON(value['addToCart']),
        'openToCart': TableItemItemAllOfAvgPositionToJSON(value['openToCart']),
        'orders': TableItemItemAllOfAvgPositionToJSON(value['orders']),
        'cartToOrder': TableItemItemAllOfAvgPositionToJSON(value['cartToOrder']),
        'visibility': TableItemItemAllOfAvgPositionToJSON(value['visibility']),
    };
}

