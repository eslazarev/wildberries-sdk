...
class TableSearchTextItemAllOfOpenToCart(BaseModel):
    ...
    percentile: StrictInt = Field(description="Процент, на который показатель конверсии в корзину выше, чем у карточек других продавцов по поисковому запросу", json_schema_extra={"examples": [50]})
