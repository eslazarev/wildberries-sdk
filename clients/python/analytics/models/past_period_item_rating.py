...
class PastPeriodItemRating(BaseModel):
    ...
    """ # noqa: E501
    start: date = Field(description="Дата начала периода. Не ранее 364 суток от вчерашнего дня и не позднее `end`", json_schema_extra={"examples": ["2026-02-08"]})
    end: date = Field(description="Дата окончания периода. Не ранее 364 суток от вчерашнего дня и не позднее даты перед началом `currentPeriod`.", json_schema_extra={"examples": ["2026-02-08"]})
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PastPeriodItemRating from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PastPeriodItemRating from a dict"""
        if obj is None:
            return None
