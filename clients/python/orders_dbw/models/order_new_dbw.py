...
class OrderNewDBW(BaseModel):
    ...
    required_meta: Optional[List[StrictStr]] = Field(default=None, description="Список идентификаторов маркировки, доступных для сборочного задания. [Указывать IMEI](./orders-dbw#tag/dbwLabelIdentifiers/paths/~1api~1v3~1dbw~1orders~1%7BorderId%7D~1meta~1imei/put) обязательно для [предмета](./work-with-products#tag/categoriesSubcategoriesAndCharacteristics/paths/~1content~1v2~1object~1all/get) `Смартфоны`, `\"subjectId\":515`", alias="requiredMeta", json_schema_extra={"examples": [["uin"]]})
