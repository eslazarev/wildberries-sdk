...
class PatchV1ClaimRequest(BaseModel):
    ...
    action: StrictStr = Field(description="Действие с заявкой.<br>Используйте одно из значений массива `actions` — ответа [метода получения заявок](./user-communication#tag/buyersReturns/operation/getV1Claims)", json_schema_extra={"examples": ["rejectcustom"]})
