...
class SizeGood(BaseModel):
    ...
    size_id: Optional[StrictInt] = Field(default=None, description="ID размера. Можно получить с помощью метода [Получение списка товаров по артикулам](./item-management#tag/pricesAndDiscounts/operation/getV2ListGoodsFilter), поле `sizeID`. В методах Контента это поле `chrtID`", alias="sizeID", json_schema_extra={"examples": [98989887]})
