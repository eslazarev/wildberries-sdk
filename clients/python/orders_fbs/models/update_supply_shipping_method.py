...
class UpdateSupplyShippingMethod(BaseModel):
    ...
    shipping_point_id: StrictInt = Field(description="ID пункта отгрузки. Можно получить с помощью [отдельного метода](./orders-fbs#tag/Postavki-FBS/operation/getV3FbsShippingPoints)", alias="shippingPointId", json_schema_extra={"examples": [100]})
    shipping_type: StrictStr = Field(description="Способ доставки до пункта отгрузки:   - `selfShipping` — доставка силами продавца   - `transportCompany` — доставка через транспортную компанию. Для этого способа обязательно [укажите ID ЭТрН](./orders-fbs#tag/Postavki-FBS/operation/patchV3FbsSuppliesWaybill) — электронной транспортной накладной — в поле `waybillUuid` ", alias="shippingType", json_schema_extra={"examples": ["transportCompany"]})
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of UpdateSupplyShippingMethod from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of UpdateSupplyShippingMethod from a dict"""
        if obj is None:
            return None
