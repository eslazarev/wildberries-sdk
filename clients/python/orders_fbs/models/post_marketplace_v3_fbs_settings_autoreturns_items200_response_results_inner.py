...
class PostMarketplaceV3FbsSettingsAutoreturnsItems200ResponseResultsInner(BaseModel):
    """
    ...
    type: Optional[StrictStr] = Field(default=None, description="Куда будет возвращён товар:   - `auto` — место возврата определяется автоматически   - `byWarehouse` — на склад WB   - `byPickupPoint` — на пункт выдачи заказов   - `byCourier` — продавцу курьером. Всегда для товаров тех [предметов](/openapi/orders-fbs#tag/autoreturnSettings/operation/getMarketplaceV3FbsSettingsAutoreturnsSubcategoriesRestricted), которые автоматически возвращаются в ПВЗ ")
    changeable: Optional[StrictBool] = Field(default=None, description="- `true` — настройки автовозврата товара можно изменить ")
    error: Optional[List[PostMarketplaceV3FbsSettingsAutoreturnsItems200ResponseResultsInnerErrorInner]] = Field(default=None, description="Детали ошибки")
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PostMarketplaceV3FbsSettingsAutoreturnsItems200ResponseResultsInner from a JSON string"""
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.error:
            for _item_error in self.error:
                _items.append(_item_error.to_dict() if _item_error is not None else None)
            _dict['error'] = _items
        return _dict

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PostMarketplaceV3FbsSettingsAutoreturnsItems200ResponseResultsInner from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "error": [PostMarketplaceV3FbsSettingsAutoreturnsItems200ResponseResultsInnerErrorInner.from_dict(_item) for _item in obj["error"]] if obj.get("error") is not None else None
        })
        return _obj
