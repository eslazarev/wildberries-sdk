...
### items (`items`)
- `items.DefaultApi.delete_v2_tag_id` — `DELETE /content/v2/tag/{id}` — Удаление ярлыка
- `items.DefaultApi.delete_v3_stocks_warehouse_id` — `DELETE /api/v3/stocks/{warehouseId}` — Удалить остатки товаров
- `items.DefaultApi.delete_v3_warehouses_warehouse_id` — `DELETE /api/v3/warehouses/{warehouseId}` — Удалить склад продавца
- `items.DefaultApi.get_v1_brands` — `GET /api/content/v1/brands` — Бренды
- `items.DefaultApi.get_v2_buffer_goods_task` — `GET /api/v2/buffer/goods/task` — Детализация необработанной загрузки
- `items.DefaultApi.get_v2_buffer_tasks` — `GET /api/v2/buffer/tasks` — Состояние необработанной загрузки
- `items.DefaultApi.get_v2_cards_limits` — `GET /content/v2/cards/limits` — Лимиты карточек товаров
...
- `items.DefaultApi.get_v2_list_goods_filter` — `GET /api/v2/list/goods/filter` — Получить товары с ценами
- `items.DefaultApi.get_v2_list_goods_size_nm` — `GET /api/v2/list/goods/size/nm` — Получить размеры товара с ценами
- `items.DefaultApi.get_v2_object_all` — `GET /content/v2/object/all` — Список предметов
- `items.DefaultApi.get_v2_object_charcs_subject_id` — `GET /content/v2/object/charcs/{subjectId}` — Характеристики предмета
- `items.DefaultApi.get_v2_object_parent_all` — `GET /content/v2/object/parent/all` — Родительские категории товаров
- `items.DefaultApi.get_v2_quarantine_goods` — `GET /api/v2/quarantine/goods` — Получить товары в карантине
- `items.DefaultApi.get_v2_tags` — `GET /content/v2/tags` — Список ярлыков
- `items.DefaultApi.get_v3_dbw_warehouses_warehouse_id_contacts` — `GET /api/v3/dbw/warehouses/{warehouseId}/contacts` — Список контактов
- `items.DefaultApi.get_v3_offices` — `GET /api/v3/offices` — Получить список складов WB
- `items.DefaultApi.get_v3_warehouses` — `GET /api/v3/warehouses` — Получить список складов продавца
- `items.DefaultApi.patch_v2_tag_id` — `PATCH /content/v2/tag/{id}` — Изменение ярлыка
...
- `items.DefaultApi.post_v2_barcodes` — `POST /content/v2/barcodes` — Генерация баркодов
- `items.DefaultApi.post_v2_cards_delete_trash` — `POST /content/v2/cards/delete/trash` — Перенос карточек товаров в корзину
- `items.DefaultApi.post_v2_cards_error_list` — `POST /content/v2/cards/error/list` — Список несозданных карточек товаров с ошибками
- `items.DefaultApi.post_v2_cards_move_nm` — `POST /content/v2/cards/moveNm` — Объединение и разъединение карточек товаров
- `items.DefaultApi.post_v2_cards_recover` — `POST /content/v2/cards/recover` — Восстановление карточек товаров из корзины
- `items.DefaultApi.post_v2_cards_update` — `POST /content/v2/cards/update` — Редактирование карточек товаров
- `items.DefaultApi.post_v2_cards_upload` — `POST /content/v2/cards/upload` — Создание карточек товаров
- `items.DefaultApi.post_v2_cards_upload_add` — `POST /content/v2/cards/upload/add` — Создание карточек товаров с присоединением
- `items.DefaultApi.post_v2_get_cards_list` — `POST /content/v2/get/cards/list` — Список карточек товаров
- `items.DefaultApi.post_v2_get_cards_trash` — `POST /content/v2/get/cards/trash` — Список карточек товаров в корзине
- `items.DefaultApi.post_v2_list_goods_filter` — `POST /api/v2/list/goods/filter` — Получить товары с ценами по артикулам
- `items.DefaultApi.post_v2_tag` — `POST /content/v2/tag` — Создание ярлыка
- `items.DefaultApi.post_v2_tag_nomenclature_link` — `POST /content/v2/tag/nomenclature/link` — Управление ярлыками в карточке товара
- `items.DefaultApi.post_v2_upload_task` — `POST /api/v2/upload/task` — Установить цены и скидки
- `items.DefaultApi.post_v2_upload_task_club_discount` — `POST /api/v2/upload/task/club-discount` — Установить скидки WB Клуба
- `items.DefaultApi.post_v2_upload_task_size` — `POST /api/v2/upload/task/size` — Установить цены для размеров
- `items.DefaultApi.post_v3_media_file` — `POST /content/v3/media/file` — Загрузить медиафайл
- `items.DefaultApi.post_v3_media_save` — `POST /content/v3/media/save` — Загрузить медиафайлы по ссылкам
- `items.DefaultApi.post_v3_stocks_warehouse_id` — `POST /api/v3/stocks/{warehouseId}` — Получить остатки товаров
- `items.DefaultApi.post_v3_warehouses` — `POST /api/v3/warehouses` — Создать склад продавца
- `items.DefaultApi.put_v3_dbw_warehouses_warehouse_id_contacts` — `PUT /api/v3/dbw/warehouses/{warehouseId}/contacts` — Обновить список контактов
- `items.DefaultApi.put_v3_stocks_warehouse_id` — `PUT /api/v3/stocks/{warehouseId}` — Обновить остатки товаров
